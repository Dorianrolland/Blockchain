...
describe("BffClient", () => {
  ...
  it("supports the embedded wallet auth and sponsored action flows", async () => {
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(
        new Response(
          JSON.stringify({
            ...
            provider: {
              id: "embedded-beta",
              label: "Embedded Wallet Beta",
              sponsoredActions: ["mint_standard", "claim_insurance"],
            },
          }),
          { status: 200, headers: { "Content-Type": "application/json" } },
        ),
    ...
    const client = new BffClient("http://localhost:8787");
    const codeRequest = await client.requestEmbeddedWalletCode("fan@example.com");
    const verifiedSession = await client.verifyEmbeddedWalletCode("fan@example.com", "123456");
    const restoredSession = await client.getEmbeddedWalletSession("header.signature");
    const action = await client.runSponsoredWalletAction("header.signature", {
      eventId: "v2-event",
      action: "claim_insurance",
      tokenId: 7n,
    });

    expect(codeRequest.devCode).toBe("123456");
    expect(verifiedSession.sessionToken).toBe("header.signature");
    expect(restoredSession?.walletAddress).toBe("0x00000000000000000000000000000000000000aa");
    expect(action.sponsoredValue).toBe(0n);
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "http://localhost:8787/v1/embedded-wallet/request-code",
      expect.objectContaining({
        method: "POST",
      }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      4,
      "http://localhost:8787/v1/embedded-wallet/actions",
      expect.objectContaining({
        method: "POST",
